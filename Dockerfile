# LUMA POD 6.1 CLEAN — no Open WebUI, no actor-gemma4, isolated Kokoro
# Repository target: Unwindology/LMMA-POD-6.1-051826
# Docker Hub target: unwindology/luma-pod-61-051826
#
# Design:
#   - vLLM comes from official vllm/vllm-openai image, not actor-gemma4.
...
FROM vllm/vllm-openai:v0.19.1-ubuntu2404
...
# OS tools only. No Open WebUI. No app-venv. No actor-gemma4.
RUN apt-get update && \
...
# LUMA Python dependencies. Keep this narrow; do not reinstall vLLM/torch.
...
RUN python3 -m venv /workspace/luma/tts/kokoro-env && \
    /workspace/luma/tts/kokoro-env/bin/pip install --no-cache-dir --upgrade pip && \
...
LABEL org.opencontainers.image.title="luma-pod-6.1-clean" \
      org.opencontainers.image.description="Clean LUMA 6.1 pod: vLLM + LUMA Gateway/RAG + JupyterLab + Tailscale + isolated Kokoro TTS. No Open WebUI." \
