...
start_tailscale () {
  ...
  if ! command -v tailscale >/dev/null 2>&1; then echo "  Tailscale: installing..."; curl -fsSL https://tailscale.com/install.sh | sh -s - >/dev/null 2>&1 || true; fi
  cleanup_tailscale_name
  ...
  disown
  sleep 5
  tailscale --socket=/tmp/tailscaled.sock up --authkey="$TAILSCALE_AUTHKEY" --hostname="$TAILSCALE_HOSTNAME" --accept-routes --accept-dns=false --force-reauth >> /workspace/logs/tailscale.log 2>&1 || {
    echo "  !! tailscale up failed — check /workspace/logs/tailscale.log"; tail -30 /workspace/logs/tailscale.log || true; return 1
  }
  local ts_ip; ts_ip="$(tailscale --socket=/tmp/tailscaled.sock ip -4 2>/dev/null || echo "pending")"
