...
# LUMA 6 repair, May 18 2026:
#   - vLLM launches only with VLLM_PY, never system Python.
#   - Kokoro launches only from /workspace/luma/tts/start_kokoro.sh.
#   - This script fails fast if vLLM is not importable from the inherited venv.

set -euo pipefail
START_T=$(date +%s)

export TMPDIR="${TMPDIR:-/tmp}"
mkdir -p "$TMPDIR" /workspace/logs /workspace/luma /workspace/luma/aethel-luma
...
# Python selection.
# Critical rule: vLLM must NOT launch with /usr/bin/python or /usr/bin/python3.
APP_PY="/workspace/app-venv/bin/python"
OPT_PY="/opt/venv/bin/python"
...
if [ -z "${VLLM_PY:-}" ]; then
  if [ -x "$APP_PY" ] && "$APP_PY" -c "import vllm" >/dev/null 2>&1; then
    VLLM_PY="$APP_PY"
  elif [ -x "$OPT_PY" ] && "$OPT_PY" -c "import vllm" >/dev/null 2>&1; then
    VLLM_PY="$OPT_PY"
  else
    echo "FATAL: vLLM is not importable from $APP_PY or $OPT_PY"
    echo "       Refusing to launch with system Python."
    exit 1
  fi
fi

export PY VLLM_PY
export PATH="$(dirname "$PY"):/workspace/app-venv/bin:/usr/local/bin:/usr/bin:/bin"

echo "============================================================"
echo " LUMA start.sh  ·  RUN_MODE=$RUN_MODE  ·  $(date)"
echo "============================================================"
echo "  Python: PY=$PY"
echo "  Python: VLLM_PY=$VLLM_PY"
"$VLLM_PY" -c "import vllm; print('  vLLM import OK from', __import__('sys').executable)"
...
ensure_readme () {
  ...
  if [ -z "${GIT_HUB_TOKEN:-}" ]; then
    if [ "$has_baked" = 1 ]; then
      echo "  README: no GIT_HUB_TOKEN, using baked default"
    else
  ...
  if curl -sfL \
    ...
    rm -f "$tmp"
    if [ "$has_baked" = 1 ]; then
      echo "  README: fetch failed, falling back to baked default"
    else
...
start_vllm () {
  ...
  echo "  vLLM: launching $MODEL_ID with $VLLM_PY"
...
start_webui () {
  ...
  if [ ! -x /workspace/app-venv/bin/open-webui ]; then
    echo "  WebUI: /workspace/app-venv/bin/open-webui not found — skipping"
    return
  fi
...
start_kokoro_tts () {
  if is_listening "$KOKORO_PORT"; then
    echo "  Kokoro TTS: already listening on $KOKORO_PORT — leaving it alone"
    return
  fi
  if [ ! -x /workspace/luma/tts/start_kokoro.sh ]; then
    echo "  Kokoro TTS: /workspace/luma/tts/start_kokoro.sh missing or not executable — skipping"
    return
  fi
  echo "  Kokoro TTS: launching on port $KOKORO_PORT"
  export KOKORO_PORT KOKORO_MODEL_DIR
  nohup /workspace/luma/tts/start_kokoro.sh \
    > "$LUMA_LOG_DIR/kokoro.log" 2>&1 &
  disown
  echo "  Kokoro TTS PID=$! — log: $LUMA_LOG_DIR/kokoro.log"
  wait_for_health "http://localhost:$KOKORO_PORT/health" "Kokoro TTS" 24 || true
}

start_luma_tools () {
  if [ -z "${LUMA_TOOLS_API_KEY:-}" ]; then
    echo "  LUMA Tools: LUMA_TOOLS_API_KEY not set — skipping"
    return
  fi
  ...
  echo "  LUMA Tools: launching luma_tools_server on :$LUMA_TOOLS_PORT"
  nohup "$PY" /workspace/luma/luma_tools_server.py \
    > "$LUMA_LOG_DIR/luma_tools_server.log" 2>&1 &
  disown
}
...
start_open_terminal () {
  ...
  "$PY" -m pip install open-terminal --quiet --break-system-packages 2>/dev/null || true
  local OT_BIN=""
