#!/usr/bin/env python3
"""
Kokoro TTS server — OpenAI-compatible speech endpoint.

Runs only inside /workspace/luma/tts/kokoro-env.
Does not touch LUMA/vLLM system Python.
"""
...
app = FastAPI(title="LUMA Kokoro TTS", version="6.1-clean")
_kokoro = None


def ensure_models():
    """Download matched v0.19 model + voices from Kokoro ONNX GitHub release."""
    ...
    for dest, url, min_size in sources:
        if dest.exists() and dest.stat().st_size >= min_size:
            continue
        if dest.exists():
            print(f"[KOKORO] {dest} is too small; re-downloading", flush=True)
            try:
                dest.unlink()
            except Exception:
                pass
...
def get_kokoro():
    ...
    if _kokoro is None:
        ...
        print("[KOKORO] Loading Kokoro model...", flush=True)
...
@app.get("/v1/models")
async def models():
    return {"object": "list", "data": [{"id": "kokoro", "object": "model", "owned_by": "luma"}]}
...
async def speech(req: SpeechRequest):
    ...
    try:
        ...
        print(f"[KOKORO] speech error: {e}", flush=True)
